...
def myModel():
    ...
    model= Sequential()
    model.add((Convolution2D(no_Of_Filters, size_of_Filter, input_shape=(imageDimesion[0], imageDimesion[1], 1), activation='relu')))  # ADDING MORE CONVOLUTION LAYERS = LESS FEATURES BUT CAN CAUSE ACCURACY TO INCREASE
    model.add((Convolution2D(no_Of_Filters, size_of_Filter, activation='relu')))
    model.add(MaxPooling2D(pool_size=size_of_pool)) # DOES NOT EFFECT THE DEPTH/NO OF FILTERS

    model.add((Convolution2D(no_Of_Filters//2, size_of_Filter2,activation='relu')))
    model.add((Convolution2D(no_Of_Filters // 2, size_of_Filter2, activation='relu')))
...
pickle_out= open("fruit.pickle","wb")  # wb = WRITE BYTE
